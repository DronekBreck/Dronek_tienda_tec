...
select p.nombre,p.precio,c.nombre from productos p inner join categorias c on c.id = p.categoria_id;

select nombre,precio from productos ;

select fecha,producto_id from ventas;

select v.fecha,p.nombre from ventas v inner join productos p on p.id = v.producto_id;